...
<main>
  ...
  <div class="breadcrumb">
    <div class="container">
      <?php 
       $separator = '<span>/</span>';
       echo '<ul>';
       echo '<li><a href="' . home_url() . '" data-translate-key="Home">Home</a>' . $separator . '</li>';
       if (is_single()) {
        ...
        if (!empty($destinations) && !is_wp_error($destinations)) {
          ...
          if ($destination->parent != 0) {
              $parent_destination = get_term($destination->parent, 'destinations');
              $parent_destination_name_en = get_field('name_en', $parent_destination) ?: $parent_destination->name;
              $parent_destination_name_jp = $parent_destination->name;
              echo '<li><a href="' . get_term_link($parent_destination) . '" class="translate" data-name-en="' . esc_attr($parent_destination_name_en) . '" data-name-jp="' . esc_attr($parent_destination_name_jp) . '">' . esc_html($parent_destination_name_en) . '</a>' . $separator . '</li>';
          }
          echo '<li><a href="' . get_term_link($destination) . '" class="translate" data-name-en="' . esc_attr($destination_name_en) . '" data-name-jp="' . esc_attr($destination_name_jp) . '">' . esc_html($destination_name_en) . '</a>' . $separator . '</li>';
        }
        $title_en = get_the_title();
        $title_jp = get_post_meta(get_the_ID(), 'title_jp', true);
        echo '<li class="translate" data-name-en="' . esc_html($title_en) . '" data-name-jp="' . esc_html($title_jp) . '">';
        echo esc_html($title_en);
        echo '</li>';
        }
        echo '</ul>';
      ?>
  ...
  <div class="tour-detail-head container">
    ...
    if (!empty($images) && is_array($images)) {
      ...
      $image_01_url = isset($image_01['url']) ? esc_url($image_01['url']) : get_template_directory_uri() . '/assets/img/ezgif-7-594f91690c.png';
      $image_02_url = isset($image_02['url']) ? esc_url($image_02['url']) : get_template_directory_uri() . '/assets/img/ezgif-7-594f91690c.png';
      $image_03_url = isset($image_03['url']) ? esc_url($image_03['url']) : get_template_directory_uri() . '/assets/img/ezgif-7-594f91690c.png';
      $image_04_url = isset($image_04['url']) ? esc_url($image_04['url']) : get_template_directory_uri() . '/assets/img/ezgif-7-594f91690c.png';
    } else {
      $image_01_url = get_template_directory_uri() . '/assets/img/noImage.png';
      $image_02_url = get_template_directory_uri() . '/assets/img/noImage.png';
